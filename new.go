...
// used by Call to lookup the Command from the internal Register (Reg)
// for completion and execution.  By convention, these must be
...
// delegate the call to a matching Command in the internal Register
// (Reg) if and only if the Command itself does not have
// a Command.Method defined. See Call for more about this delegation on
// how it finds key name matches in the internal Register (Reg).
//
...
// to other other Commands in the cmdbox.Reg (but avoid unnecessary
...
// Commands in the cmdbox.Reg that must eventually be populated by
...
// predicted at init() time. Not all Commands may yet have been
// registered before any other cmdbox.New is called. This means runtime
// testing is required to check for errant calls to unregistered
...
// conflicts when importing Commands into the internal Register (Reg)
// duplicates are still a possibility. Rather than override those
// previously added any identical duplicate will simply have an
// underscore added to the name. Since the processing of init functions
// is gauranteed to happen in consistent fashion for any given
// composition this allows rare naming conflicts to be resolved in the
// main init before calling Execute when needed.
//
